...
func TestRequirementServiceRoleAndUpdate(t *testing.T) {
	...
	reqSvc := NewRequirementService(db, repository.NewRequirementRepository(db), repository.NewBidRepository(db), notifSvc, logSvc, discardLogger())
...
func TestBidServiceCreateAndWithdraw(t *testing.T) {
	...
	notifSvc := NewNotificationService(repository.NewNotificationRepository(db), discardLogger())
	reqSvc := NewRequirementService(db, repository.NewRequirementRepository(db), repository.NewBidRepository(db), notifSvc, logSvc, discardLogger())
	bidSvc := NewBidService(db, repository.NewBidRepository(db), repository.NewRequirementRepository(db), notifSvc, logSvc, discardLogger())
